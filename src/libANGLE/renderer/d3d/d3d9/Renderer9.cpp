...
egl::Error Renderer9::initializeFullscreen(EGLNativeWindowType win)
{
	TRACE_EVENT0("gpu.angle", "GetModuleHandle_d3d9");
	mD3d9Module = GetModuleHandle(TEXT("d3d9.dll"));
	...
	{
		TRACE_EVENT0("gpu.angle", "D3d9_CreateDevice");
		result = mD3d9->CreateDevice(mAdapter, mDeviceType, win, behaviorFlags | D3DCREATE_HARDWARE_VERTEXPROCESSING | D3DCREATE_PUREDEVICE, &presentParameters, &mDevice);
	}
	if (result == D3DERR_OUTOFVIDEOMEMORY || result == E_OUTOFMEMORY || result == D3DERR_DEVICELOST)
	{
		ASSERT(false);
	...
	if (FAILED(result))
	{
		...
		TRACE_EVENT0("gpu.angle", "D3d9_CreateDevice2");
		result = mD3d9->CreateDevice(mAdapter, mDeviceType, win, behaviorFlags | D3DCREATE_SOFTWARE_VERTEXPROCESSING, &presentParameters, &mDevice);
